...
class ReviewLintCommand(sublime_plugin.TextCommand):
	...
	def check(self, view, block_commands, inline_command):
		...
		for block_command in block_commands:
			...
			for region in regions:
				m = re.search("^//" + block_command + "\[([^]]*)\]", view.substr(region))
		...
		error_message = ""
		for region in regions:
			m = re.search("@<" + inline_command + ">\{([^}]+)\}", view.substr(region))
			key = m.group(1)
			if key not in defined_list_keys:
				(row, col) = view.rowcol(region.a)
				error_message += (str(row + 1) + ": Error key not found (" + self.view.substr(region) + ")\n")
	...
	def checkImage(self, view):
		...
		for region in regions:
			...
			(row, col) = view.rowcol(region.a)
			error_message += (str(row + 1) + ": Error image not exists (" + self.view.substr(region) + ")\n")
